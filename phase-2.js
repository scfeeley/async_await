...
async function workout(totalTime) {
  try{
    let timeRem = await stretch(totalTime);
    timeRem = await runOnTreadmill(timeRem);
    timeRem = await liftWeights(timeRem)
    console.log(`done working out with ${ timeRem / 1000 } seconds left`);
  }catch(err){
